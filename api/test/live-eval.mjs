...
const repeatPauseMs = runCount > 1 ? 7_000 : 0;
...
for (let run = 1; run <= runCount; run += 1) {
  for (const manifestPath of manifestPaths.sort()) {
    if (repeatPauseMs > 0 && requestNumber > 0) await delay(repeatPauseMs);
    requestNumber += 1;
...
console.log(JSON.stringify({ target, runs: runCount, cases: results }, null, 2));
